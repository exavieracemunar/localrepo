#!/bin/bash
		...
		set(){
	       ...
	       s=$(service crond start > /dev/null 2>&1)
               c=$(crontab -l > /dev/null 2>&1 | grep -v -F "$croncmd" ; echo -e "$mail\n$path\n$cronjob" | crontab - )
		...
		pass="Toro_sysad1"
		backup=$(mysqldump -u root -p$pass --databases important_files > /opt/dump-backup/important_files.sql > /dev/null 2>&1)
		...
		$(tar cvf /opt/dump-backup.tar /opt/dump-backup > /dev/null 2>&1) #Archive the folder dump-backup
			...
			setbackup(){
				...
				#incremental backup
				rsync -avr --numeric-ids /opt/dump-backup /opt/incremental-backup/$TIMEDATE > /dev/null 2>&1
				#full backup
				rsync -avr /opt/dump-backup /opt/full-backup > /dev/null 2>&1
